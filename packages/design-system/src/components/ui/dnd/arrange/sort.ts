...
import type { DndAxis, DndDelta, DndId, DndPoint, DndRect, ItemSnapshot, ListSnapshot } from "../types"
...
export function computeSortTransforms(
  ...
  if (!over) {
    for (const item of items) {
      if (item.id === sourceId || item.listId !== source.listId)
        continue
...
export function placeholderRect(
  ...
  if (over.index >= members.length) {
    ...
    if (draggingDownInSource)
      return { x: last.x, y: last.y, ...size }
  ...
  if (over.listId === source.listId && over.index > source.index) {
    ...
    if (shifted)
      return { x: shifted.x, y: shifted.y, ...size }
  }
