...
test("hitTestSort returns the remaining-list insert index", () => {
  expect(hitTestSort(cards, [column], { x: 10, y: 5 }, ["b"])).toEqual({ listId: "todo", index: 0 })
  expect(hitTestSort(cards, [column], { x: 10, y: 66 }, ["b"])).toEqual({ listId: "todo", index: 2 })
  expect(hitTestSort(cards, [column], { x: 10, y: 180 }, ["b"])).toEqual({ listId: "todo", index: 3 })
})
...
test("same-list append uses the last remaining item's vacated box, not a slot after it", () => {
  ...
  const fourEnd = placeholderRect(cards, cards[1]!, { listId: "todo", index: 3 }, column, "vertical")
