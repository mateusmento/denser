...
const placeholderStyle = computed(() => {
  ...
  const element = host();
  if (!view || view.listId !== props.listId || !element) return { display: "none" as const };
...
const resolvedPlaceholderAs = computed(() => {
  if (props.placeholderAs) return props.placeholderAs;
  const tag = host()?.tagName;
  if (tag === "UL" || tag === "OL" || props.as === "ul" || props.as === "ol") return "li";
  return "div";
});
</script>
