...
export const bubbleVariants = cva(
  cn([
    "max-w-[80%] gap-1 data-[align=end]:self-end data-[variant=ghost]:max-w-full",
    "group/bubble relative flex w-fit min-w-0 flex-col group-data-[align=end]/message:self-end",
  ]),
);
...
export const bubbleContentVariants = cva(
  cn([
    ...
    "group-data-[align=end]/bubble:self-end [button,a]:focus-visible:ring-ring/30",
    "w-fit max-w-full min-w-0 overflow-hidden wrap-break-word [button]:text-left [button,a]:transition-colors",
  ]),
  {
    variants: {
      variant: {
        default: cn(["bg-primary text-primary-foreground", "[&:is(button,a):hover]:bg-primary/80"]),
        secondary: cn([
        ...
        ghost: cn([
          ...
          "bg-transparent hover:bg-mist-50 dark:hover:bg-secondary",
          "data-highlighted:bg-mist-50 dark:data-highlighted:bg-secondary",
          "hover:shadow-[0_1px_1px_var(--color-mist-200)]",
          "data-highlighted:shadow-[0_1px_1px_var(--color-mist-200)]",
          "dark:hover:shadow-[0_1px_1px_var(--color-mist-950)]",
          "dark:data-highlighted:shadow-[0_1px_1px_var(--color-mist-950)]",
          "border border-transparent hover:border-border data-highlighted:border-border",
          "[&:is(button,a):hover]:text-foreground",
...
export const [injectBubbleContext, provideBubbleContext] = createContext<BubbleContext>("Bubble");

export const bubbleReactionsVariants = cva(
  cn([
    "absolute z-10 shrink-0 gap-1 rounded-full bg-muted px-1.5 py-0.5 text-sm ring-3 ring-card has-[button]:p-0",
