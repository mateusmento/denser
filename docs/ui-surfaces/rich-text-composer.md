...
2. In Suggestion `render`, mount **RichTextSuggestionMenu** and call **`props.mount(element)`** — TipTap then positions with Floating UI (`placement`, `offset`, `flip`, `autoUpdate`). Extra middleware (`shift`, `size`) via Suggestion `floatingUi.middleware` imported from `@floating-ui/dom`.
3. Call the returned `unmount` in `onExit`.
4. Manual `computePosition(reference, el, props.floatingUi)` only as an escape hatch (no scroll following unless we add `autoUpdate` ourselves).
...
| `codeBlock` | `rt-code-block` | Chrome + language control in composer |
...
Every derived composer and every **RichTextSubtree** **must** understand this vocabulary. Hosts may add slash items and action-row entry points. They must not fork a smaller or larger *schema* for the same document.
